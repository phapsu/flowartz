...
class Workshop_model extends CI_Model {
    ...
    public function save($postdata, $gallery) {
    ...
    public function get_workshop($id, $admin=false) {
        if (!$admin) {
            $query = $this->db->query('SELECT w.*, u.email as email FROM fa_workshops as w left join fa_users as u on w.uid = u.uid WHERE w.status=0 and w.wid = ' . $id);
        } else {
            $query = $this->db->query('SELECT w.*, u.email as email FROM fa_workshops as w left join fa_users as u on w.uid = u.uid WHERE w.wid = ' . $id);
        }
    ...
    public function get_workshop_nearby() {
        ...
        $sql = "SELECT w.* FROM fa_workshops as w WHERE w.location like '%Toronto%'";
        $sql .=' order by w.date desc limit 4';
    ...
    public function delete_image($postdata) {
        $data = array(
            'image' => ''
        );
        ...
        $path = './application/files/';
        //delete file
        @unlink($path . $postdata['image_name']);
